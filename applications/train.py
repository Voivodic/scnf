...
def get_data_loaders(n_samples, config):
    ...
    grid_file = (
        h5py.File(config["paths"]["grid_path"], "r")
        if config["paths"]["grid_path"]
        else None
    )
    array_file = (
        h5py.File(config["paths"]["array_path"], "r")
        if config["paths"]["array_path"]
        else None
    )

    theta_dataset = jnp.array(theta_file["thetas"])
    grid_dataset = jnp.array(grid_file["grids"]) if grid_file else None
    array_dataset = jnp.array(array_file["arrays"]) if array_file else None

    # Get the number of samples and check if the datasets are compatible
    if theta_dataset.shape[0] < n_samples:
        raise ValueError(
            "The number of samples in thetas.hdf5 is less than the required number of samples."
        )
    if grid_dataset is not None and grid_dataset.shape[0] < n_samples:
        raise ValueError(
            "The number of samples in grids.hdf5 is less than the required number of samples."
        )
    if array_dataset is not None and array_dataset.shape[0] < n_samples:
        raise ValueError(
            "The number of samples in arrays.hdf5 is less than the required number of samples."
        )
    ...
    grid_mean, grid_std = (
        get_normalization_stats(grid_dataset, n_train)
        if grid_dataset
        else (0, 1)
    )
    array_mean, array_std = (
        get_normalization_stats(array_dataset, n_train)
        if array_dataset
        else (0, 1)
    )
    ...
    def data_loader_theta(index):
        return normalize(
            jnp.array(theta_dataset)[index, :], theta_mean, theta_std
        )

    def data_loader_grid(index):
        if grid_dataset is not None:
            return normalize(
                jnp.array(grid_dataset)[index, :], grid_mean, grid_std
            )
        return jnp.array([])
    ...
    def data_loader_array(index):
        if array_dataset is not None:
            return normalize(
                jnp.array(array_dataset)[index, :], array_mean, array_std
            )
        return jnp.array([])
...
def save_normalizations(config, normalization_stats):
    """
    ...
    """
    output_path = f"{config['paths']['output_folder']}/normalization_{config['paths']['suffix']}.hdf5"
    ...
    print(f"Normalizations saved to {output_path}")
...
def main(config):
    ...
    """
    # Get the parameters needed for the training
    n_samples = config["data_params"]["n_samples"]
    dim = config["data_params"]["dimensions"]
    ...
    data_loader_theta, data_loader_grid, data_loader_array, norm_stats = (
        get_data_loaders(n_samples, config)
    )
    ...
    save_normalizations(config, norm_stats)
